import { LayoutAdmin, Table, Button, StatusPill } from "@/components";
import { useMemo, useState } from "react";
import * as Bs from "react-icons/bs";
import { useQuery } from "@apollo/client";
import { GET_USERROLE } from "@/apollo/queries";
const Guests = () => {
  const [guests, setGuests] = useState(null);
  const { loading, error, data } = useQuery(GET_USERROLE, {
    variables: { role: "guests" },
  });
  if (data && !guests) {
    setGuests(data.userRole);
  }
  const Guest = useMemo(() => {
    if (!guests) {
      return [];
    } else {
      return guests;
    }
  }, [guests]);
  const columns = useMemo(
    () => [
      {
        Header: "Name",
        accessor: "firstName",
        lastNameAccessor: "lastName",
        imgAccessor: "image",
        emailAccessor: "email",
        Cell: ({ value, column, row }) => {
          const imgUrl = row.original[column.imgAccessor]
            ? row.original[column.imgAccessor]
            : "/img/avatar/default.png";
          return (
            <div className="flex items-center">
              <div className="flex-shrink-0 h-10 w-10">
                <img
                  className="h-10 w-10 rounded-full"
                  src={imgUrl}
                  alt={`avatar-${value}`}
                />
              </div>
              <div className="ml-4">
                <div className="text-sm font-medium text-gray-900">{`${value} ${row.original[column.lastNameAccessor]
                  } `}</div>
                <div className="text-sm text-gray-500">
                  {row.original[column.emailAccessor]}
                </div>
              </div>
            </div>
          );
        },
      },
      {
        Header: "Status",
        accessor: "active",
        Cell: ({ value }) => {
          const status = value;
          return (
            <span
              className={`${status
                ? "bg-green-100 text-green-700"
                : "bg-yellow-100 text-yellow-700"
                } px-3 py-1 uppercase leading-wide font-bold text-xs rounded-full shadow-sm`}
            >
              {status ? "Active" : "Inactive"}
            </span>
          );
        },
      },
      {
        Header: "Role",
        accessor: "role",
      },
      {
        Header: "Action",
        Cell: ({ row }) => (
          <div>
            <button
              type="button"
              className="px-2 py-2 outline-none bg-yellow-500 text-white hover:bg-yellow-400 rounded-sm text-sm"
              onClick={() => setEditModal(true)}
            >
              <Bs.BsWrench className="inline-block mr-1 text-base" />
              Edit
            </button>{" "}
            <button
              type="button"
              className="px-2 py-2 outline-none bg-red-600 text-white hover:bg-red-500 rounded-sm text-sm"
            >
              <Bs.BsTrash className="inline-block mr-1 text-base" />
              Delete
            </button>
          </div>
        ),
      },
    ],
    []
  );
  return (
    <LayoutAdmin>
      <section className="container mt-12">
        <div className="flex justify-between p-2">
          <h1 className="text-2xl font-bold block">Guests</h1>
          <h1 className="text-sm font-bold mt-8 pr-2">
            <span className="text-gray-400">admin {"> "}</span>
            Guests
          </h1>
        </div>
        <div className="float-right mr-7">
          <Button
            className={"bg-blue-600 hover:bg-blue-400 text-white"}
            onClick={() => setOpenModal(true)}
          >
            <Bs.BsFileEarmarkPlus className="text-white mr-1 text-lg" />
            Add Guests
          </Button>
        </div>
        <div className="w-[80vw] p-5">
          <Table columns={columns} data={Guest} />
        </div>
      </section>
    </LayoutAdmin>
  );
};

export default Guests;
