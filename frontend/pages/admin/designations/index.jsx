...
import { useGetDesignations, useGetDesignation, useCreateDesignation, useUpdateDesignation, useDeleteDesignation } from "@/apollo/action";
const Designations = () => {
  const [modalNew, setModalNew] = useState(false);
  const [modalEdit, setModalEdit] = useState(false);
  const [ID, setID] = useState("");
  const { loading, error, data: designationsData } = useGetDesignations();
  const [createDesignation, { error: errorCreate, data: departementsNew }] = useCreateDesignation();
  const [getDesignation, { error: errorGet, data: designationGet }] = useGetDesignation();
  const [updateDesignation, { error: errorUpdate }] = useUpdateDesignation();
  ...
  useEffect(() => {
    if (ID !== "") {
      if (ID) {
        getDesignation({ variables: { id: ID } })
        console.log(ID)
      }
    }
  }, [ID, designationGet])
        ...
        Cell: ({ row }) => (
          <div>
            ...
            <button
              ...
              onClick={() => deleteDesignation({ variables: { id: row.original._id } })}
            >
  ...
  const handleNewDesignation = (input) => {
    createDesignation({ variables: input });
    setModalNew(false);
  }
  const handleEditDesignation = (data) => {
    const { id, nameDesignation, active } = data;
    const input = {
      nameDesignation,
      active,
    }
    updateDesignation({ variables: { id, input } });
    setModalEdit(false);
  }
  return (
    <LayoutAdmin>
      <section className="container mt-12">
        <div className="flex justify-between p-2">
          <h1 className="text-2xl font-bold block">Designations</h1>
        ...
        <div className="w-[80vw] p-5">
          ...
          {!loading && !error && (
            <Table columns={columns} data={(designationsData && designationsData.designations) || []} />
          )}
        </div>
      </section>
      ...
      <Modal openModal={modalEdit} closeModal={closeModalEdit}>
        ...
        <div className="max-h-96 overflow-y-scroll p-4">
          <FormDesignation
            ...
            initialdata={designationGet && designationGet.designation}
          />
        </div>
      </Modal>
    </LayoutAdmin>
